## File: WORKING_DIRECTORY/scripts/dgp/dgp308.R
...
name <- "data8''"
description <- "Data 6'' from Su & White (2014)"
sample.data <- function(n) {
    ...
    for(i in 4:(burn.in + n)) {
        ...
        ret[i,1] <- 0.5*ret[i-1,1] + 0.25*ret[i-2,1] + 0.125*ret[i-3,1] + 0.5*ret[i, 2]*rnorm(1)
        ret[i,3] <- ret[i-1,1]
        ret[i,4] <- ret[i-2,1]
        ret[i,5] <- ret[i-3,1]
    }
