...
(\#eq:trans)
\end{equation}
The transformation enables us to simplify the local Gaussian representation \@ref(eq:localgaussian) by writing the density $f_{\Z}$ of $\Z$ as 
\begin{equation}
f_{\Z}(\z) = \psi(\z, R(\z)) = \frac{1}{|2\pi\R(\z)|^{1/2}}\exp\left\{-\frac{1}{2}\z^T\R^{-1}(\z)\z\right\},
(\#eq:lgdeapprox)
\end{equation}  
where, as in Otneim and Tjøstheim [-@otneim2017locally; -@otneim2017conditional] in a simplified representation, we have fixed local means and standard deviations $\mu_j(\z) = 0$ and $\sigma_j^2(\z) = 1$, $j = 1,\ldots,p$, and where $\R(\z) = \{\rho_{jk}(\z)\}$ is the *local correlation matrix*.
...
We will in this paper refer to $\X$ and its probability density function $f_{\X}$ as being on the \(x\)-scale, and to $\Z$ and its probability density function $f_{\Z}$ as being on the \(z\)-scale. For further discussion of the simplified $z$-representation we refer to @otneim2017locally.
...
Equations \@ref(eq:matrixlocalpartial), \@ref(eq:definition) and their empirical counterparts \@ref(eq:localpartialcov) and \@ref(eq:estimate) demonstrate clearly that the LGPC is nothing more than a deterministic function of the local correlation matrix, in the same way as the ordinary partial correlation is a function of the ordinary correlation matrix. The asymptotic behavior of $\halpha(\z)$ can thus be derived directly from the asymptotic behavior of $\hR(\z)$, which, for the most part, has been established in earlier works, see e.g. @tjostheim2013local and @otneim2017locally. Some key results follow below, and we refer to the supplementary material, Sections B--D for proofs of the asymptotic distributional results in this subsection, and to Section A.3 for a discussion of more general situations and for a result on consistency.
...
5. The non-negative kernel function $K(\cdot)$ sasisfies $\sup_{\z}K(\z) < \infty$, $\int K(\y)\,\textrm{d}\y < \infty$, $\partial/\partial z_iK(\z) <\infty$, and $\lim_{z_i \rightarrow \infty}|z_iK(\z)| = 0$ for $i = 1,2,3$.
...
\begin{figure}
    \subfloat[$n = $ 100\label{fig:gaussian-example-plots1}]{\includegraphics[width=0.31\linewidth]{figures/gaussian-example-plots-1} }
    \subfloat[$n = $ 500\label{fig:gaussian-example-plots2}]{\includegraphics[width=0.31\linewidth]{figures/gaussian-example-plots-2} }
    \subfloat[$n = $ 2000\label{fig:gaussian-example-plots3}]{\includegraphics[width=0.31\linewidth]{figures/gaussian-example-plots-3} }
...
\begin{figure}[p]
    \subfloat[Observed pairs $(X_1, X_2)$ from the conditionally \newline Gaussian model.\label{fig:simex2-plots1}]{\includegraphics[width=0.49\linewidth]{figures/simex2-plots-1} }
    \subfloat[The estimated LGPC between $X_1$ and $X_2$ given $\rho = -0.9$.\label{fig:simex2-plots2}]{\includegraphics[width=0.49\linewidth]{figures/simex2-plots-2} }\newline
    \subfloat[The estimated LGPC between $X_1$ and $X_2$ \newline  given $\rho = 0$\label{fig:simex2-plots3}]{\includegraphics[width=0.49\linewidth]{figures/simex2-plots-3} }
    \subfloat[The estimated LGPC between $X_1$ and $X_2$ \newline given $\rho = 0.9$\label{fig:simex2-plots4}]{\includegraphics[width=0.49\linewidth]{figures/simex2-plots-4} }
...
The first set of simulation results are collected from @su2008nonparametric: In Table \ref{tab:n100} LIN is a standard linear Granger causality test, CM and KS are tests by @linton1997conditional that use statistics of the two versions of the nonparametric test developed by @delgado2001significance. HEL refers to the Hellinger distance test that @su2008nonparametric present. Next, we move to @su2014testing, who provide simulations for their test for conditional independence based on the empirical likelihood (SEL), as well as the test by @su2007consistent that is based on properties of the conditional characteristic function (CHF). @cheng2012conditional provide simulations for their tests based on the maximal conditional correlation (MCC). In the online supplement to this article we provide a richer version of Table \@ref(tab:n100) below, namely Table 2*, that includes more results for the HEL- and MCC-tests at different levels of smoothing.
...
There is a potential for much further work. A next natural step may be structural equations as used in network analysis and Pearl-type causality [@pearl2000causality], and ultimately further exploring the relationship or lack of relationship between that type of causality and Granger causality.

All methods presented in this paper have been implemented in the R programming language [@r], and is available in the package **lg** [@otneim2019lg]. In addition, we provide code and data for easy replication of the results presented here in an online appendix.

**Acknowledgements.** The authors gratefully acknowledge the editor as well as two anonymus referees, whose comments have greatly improved this paper.
