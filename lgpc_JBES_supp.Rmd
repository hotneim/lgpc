...
All of these parameters can be described and estimated locally in the exact same way as laid out in the main article, resulting from the locally Gaussian joint density $\psi(\z,\fv)$ at the point $\fv = \z$,
...
\end{equation}
see eq. (22) in the main article, and see Theorem 2 in Otneim and Tjøstheim (2018) with $\fSigma = \fLambda$ for a definition of $\fLambda$. The computation of $\nabla g$ is outlined in Section \ref{app-asvar}.
\end{itemize}
...
In the original version of this proof we used an argument based on the law of the iterated logarithm of @rio1995functional, leading to a rate of $n^{-1/2}(2 \log \log n )^{1/2}$, but an anynomous referee provided us with a much simpler argument based on a U-statistic decomposition for a strongly mixing process. In the following we omit the index $i=1,2$:

\begin{align*}
\frac{1}{n}\sum_{i=1}^n \left(F_n(X_{i,t}) - F(X_{i,t})\right) &= 
\frac{1}{n^2}\sum_{i=1}^n\sum_{j\neq i}^n \left(I(X_{j, t} \leq X_{i,t}) - F(X_{i,t})\right) + \frac{1}{n^2}\sum_{i=1}^n \left(1 - F(X_{i,t}\right) \\
&= \frac{1}{n} \sum_{i=1}^n \left(0.5 - F(X_{i,t})\right) + O_p\left(n^{-1}\right) \\
